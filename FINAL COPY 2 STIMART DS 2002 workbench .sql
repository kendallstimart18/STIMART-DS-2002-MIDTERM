...
-- combining Mongo with SQL customer_key
SELECT 
    mc.customer_key,
    mc.account_number,
    mc.customer_type,
    fs.sales_order_id,
    fs.sales_amount
FROM mongo_customer AS mc
JOIN fact_sales AS fs
    ON mc.customer_key = fs.customer_key
LIMIT 20;

-- combining dim_product with fact_sales
SELECT
    p.product_key,
    p.product_name,
    fs.sales_order_id,
...
-- combining csv file with SQL table
SELECT
    sp.salesperson_id,
    sp.first_name,
    sp.last_name,
